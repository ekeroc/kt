...
#include "../inc/dms_utest_mod_vol_mgm.h"
...
static void dms_utest_vol_mgm_show_cb( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T   ret = DMS_ERR_CODE_SUCCESS;
	
	if ( ioctl_data.mod_id != CCMAKDBG_GLOBAL_MOD_ID_VOLUME_MGM )
	{
...
#if 0
static void DMS_UTEST_MOD_VOL_MGM_if_query_ow_cb( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T          ret = DMS_ERR_CODE_SUCCESS;
...
static void dms_utest_vol_mgm_if_set_dbg_flag( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T   ret = DMS_ERR_CODE_SUCCESS;
    unsigned int     *flag;
...
{	
	...
	ret = CCMAKDBG_CDEV_IF_Register_Callback( CCMAKDBG_GLOBAL_MOD_ID_VOLUME_MGM,
	                                          CCMAKDBG_GLOBAL_VOL_MGM_Create_Vol,
	                                          dms_utest_vol_mgm_show_cb,
	                                          sizeof(long long));
    if ( ret != CCMAKDBG_ERR_CODE_SUCCESS )
    {
        printk("DMS_UTEST_MOD_VOL_MGM: In function %s at line %d failer ret=%d \n", __FUNCTION__, __LINE__, ret );
        return ret;
    }

	ret = CCMAKDBG_CDEV_IF_Register_Callback( CCMAKDBG_GLOBAL_MOD_ID_VOLUME_MGM,
	                                          CCMAKDBG_GLOBAL_VOL_MGM_Atta_Vol,
	                                          dms_utest_vol_mgm_show_cb,
	                                          sizeof(long long));
    if ( ret != CCMAKDBG_ERR_CODE_SUCCESS )
    {
	...
	CCMAKDBG_CDEV_CBF_TBL_Show_Table(CCMAKDBG_GLOBAL_MOD_ID_VOLUME_MGM);
    return ret;
}
