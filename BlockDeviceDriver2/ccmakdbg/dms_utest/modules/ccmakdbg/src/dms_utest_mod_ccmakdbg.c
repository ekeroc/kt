...
#include "../inc/dms_utest_mod_ccmakdbg.h"
...
static void dms_utest_ccmakdbg_show_cb( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T   ret = DMS_ERR_CODE_SUCCESS;
	
	if ( ioctl_data.mod_id != CCMAKDBG_GLOBAL_MOD_ID_CCMAKDBG )
	{
...
#if 0
static void DMS_UTEST_MOD_CCMAKDBG_if_query_ow_cb( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T          ret = DMS_ERR_CODE_SUCCESS;
...
static void dms_utest_ccmakdbg_if_set_dbg_flag( CCMAKDBG_CDEV_IF_IOCTL_CMD  ioctl_data )
{
    DMS_ERR_CODE_T   ret = DMS_ERR_CODE_SUCCESS;
    unsigned int     *flag;
...
{	
	...
	ret = CCMAKDBG_CDEV_IF_Register_Callback( CCMAKDBG_GLOBAL_MOD_ID_CCMAKDBG,
	                                          CCMAKDBG_GLOBAL_KDBG_Showall_Func,
	                                          dms_utest_ccmakdbg_show_cb,
	                                          sizeof(long long));
    if ( ret != CCMAKDBG_ERR_CODE_SUCCESS )
    {
        printk("DMS_UTEST_MOD_CCMAKDBG: In function %s at line %d failer ret=%d \n", __FUNCTION__, __LINE__, ret );
        return ret;
    }

	CCMAKDBG_CDEV_CBF_TBL_Show_Table(CCMAKDBG_GLOBAL_MOD_ID_CCMAKDBG);
    return ret;
}
