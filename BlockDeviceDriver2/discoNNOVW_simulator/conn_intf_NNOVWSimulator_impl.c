 ...
 *
 * conn_intf_NOVWNSimulator_impl.c
 *
...
static void nnovwSmltorImpl_connect_target (discoC_conn_t *myconn)
{
    cs_wrapper_t *sk_wrapper;
    NNOVWSimulator_t *nnovwSmltor;
    conn_action_t action;

    nnovwSmltor = create_NNOVWSmltor(myconn->ipaddr, myconn->port);
    if (IS_ERR_OR_NULL(nnovwSmltor)) {
        return;
    }
    ...
    if (CONN_A_NoAction == action) {
        dms_printk(LOG_LVL_WARN,
                "DMSC WARN NNOVWSmltor fail create conn transit CONN_E_Connect %s:%d\n",
                myconn->ipv4addr_str, myconn->port);
    }

    //this function will connect to target server socket
    sk_wrapper = _conn_target(myconn, nnovwSmltor);
    if(IS_ERR_OR_NULL(sk_wrapper)) {
        dms_printk(LOG_LVL_WARN,
                "DMSC WARN NNOVWSmltor fail create sock wrapper %s:%d\n",
                myconn->ipv4addr_str, myconn->port);
        return;
    }
    ...
    if (action != CONN_A_DO_Service) {
        dms_printk(LOG_LVL_WARN,
                "DMSC WARN NNOVWSmltor fail change conn state: CONN_E_Connect_OK\n");
        return;
    }
    ...
    if (myconn->conn_done_fn(myconn, false)) {
        dms_printk(LOG_LVL_WARN,
                "DMSC WARN NNOVWSmltor conn_don_fn fail after re-connect to %s:%d success\n",
                myconn->ipv4addr_str, myconn->port);
        //TODO: error handling whe socket is ok but fail to create rx
    }
}

static int32_t nnovwSmltorImpl_tx_send (discoC_conn_t *conn, void *buf, size_t len,
               const bool lock_sock, const bool do_simulation)
{
    ...
    ret = discoNNOVWSrv_rx_rcv((NNOVWSimulator_t *)sock_wrapper->private_data, len, buf);
...
static int32_t nnovwSmltorImpl_rx_rcv (cs_wrapper_t *sock_wrapper, void *buf, size_t len)
{
    return discoNNOVWSrv_tx_send(sock_wrapper->private_data, len, buf);
}

static void _nnovwSmltor_free_socket (cs_wrapper_t *sock)
{
    int32_t retry_free;
...
static void nnovwSmltorImpl_close_conn (discoC_conn_t *myconn, cs_wrapper_t *old_sock)
{
    if (!IS_ERR_OR_NULL(old_sock)) {
        stop_NNOVWSmltor((NNOVWSimulator_t *)old_sock->private_data);
    }
    ...
    _nnovwSmltor_free_socket(old_sock);

    dms_printk(LOG_LVL_INFO, "DMSC INFO NN OVW Simulator not support close conn\n");
}
 ...
 */
static int32_t nnovwSmltorImpl_reset_conn (discoC_conn_t *myconn)
{
    int32_t ret;

    ret = 0;

    dms_printk(LOG_LVL_INFO, "DMSC INFO NNOVW Simulator not support reset conn\n");

    return ret;
}

static void nnovwSmltorImpl_reconnect (discoC_conn_t *myconn)
{
    dms_printk(LOG_LVL_INFO, "DMSC INFO NNOVW Simulator not support reconnect\n");
}

void NNOVWSimulator_set_connIntf (conn_intf_fn_t *myIntf)
{
    myIntf->rx_rcv = nnovwSmltorImpl_rx_rcv;
    myIntf->tx_send = nnovwSmltorImpl_tx_send;
    myIntf->resetConn = nnovwSmltorImpl_reset_conn;
    myIntf->reconnect = nnovwSmltorImpl_reconnect;
    myIntf->closeConn = nnovwSmltorImpl_close_conn;
    myIntf->connTarget = nnovwSmltorImpl_connect_target;
}
